...
fn main() {
    ...
    for sym in analysis.symbol_index().all_symbols() {
        println!("  {} (short_name: {:?}) at line {} col {}-{}", 
        ...
        if let (Some(sn_start_line), Some(sn_start_col), Some(sn_end_line), Some(sn_end_col)) = 
            (sym.short_name_start_line, sym.short_name_start_col, sym.short_name_end_line, sym.short_name_end_col) {
            println!("    short_name_span: line {} col {}-{}:{}", sn_start_line, sn_start_col, sn_end_line, sn_end_col);
        }
    }
