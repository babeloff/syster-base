...
fn main() {
    ...
    for sym in analysis.symbol_index().all_symbols() {
        for trk in &sym.type_refs {
            for tr in trk.as_refs() {
                if &*tr.target == "MySignal" && tr.resolved_target.as_deref() == Some("Test::MySignal") {
                    found_my_signal = true;
                }
                if &*tr.target == "OtherSignal" && tr.resolved_target.as_deref() == Some("Test::OtherSignal") {
    ...
    println!("MySignal from 'accept MySignal' resolved: {}", found_my_signal);
    println!("OtherSignal from 'send new OtherSignal()' resolved: {}", found_other_signal);
