            ...
            for trk in &sym.type_refs {
                for tr in trk.as_refs() {
                    if tr.target.as_ref() == "medium" {
                        println!(
                            "level's type_ref 'medium': resolved_target = {:?}",
                            tr.resolved_target
                        );
                    }
                }
    ...
    if let Some(h) = &hover {
        ...
        for sym in index.symbols_in_file(file_id) {
            ...
            for trk in &sym.type_refs {
                for tr in trk.as_refs() {
                    println!(
                        "  TypeRef: target='{}' kind={:?} lines={}-{} cols={}-{}",
        ...
        let lib_file_id = analysis
            .get_file_id("lib.sysml")
        ...
        match resolver2.resolve("medium") {
            syster::hir::ResolveResult::Found(sym) => {
                println!(
                    "In engine scope, resolved 'medium' to: {}",
                    sym.qualified_name
                );
            }
            syster::hir::ResolveResult::NotFound => {
